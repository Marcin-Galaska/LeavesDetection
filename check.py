...
def main():
    student_id = '151227'  # Tutaj należy wpisać swój numer indeksu
    ...
    with (ZipFile(data, 'w', ZIP_DEFLATED) as zip_file):
        ...
        for file in base_path.rglob('*'):
            if(
                    file.name in ('data.yaml', 'best.pt', 'train.py', 'yolov8x.pt') or
                    file.is_relative_to(base_path / 'venv') or
                    file.is_relative_to(base_path / '.idea') or
                    file.is_relative_to(base_path / 'runs') or
                    file.is_relative_to(base_path / 'datasets')
            ):
                continue
